...
namespace Castle
{
    ...
    internal class ItemManager
    {
        ...
        private void CreateCastleItems()
        {
            ...
            castleFurnatureItems.Add(new CastleFurnature("a Bed.", " It's Old.", "BEDS", 10, "Bed"));
        ...
        private UserMessage RunSpecialCommand(CommandVerb verb, String subject, Point location)
        {
            ...
            foreach (var item in currentInventory)
            {
                ...
                foreach(var name in item.parsingNames)
                {
                    if(name.StartsWith(subject, StringComparison.CurrentCultureIgnoreCase))
                    {
                        foundItem = true;
                        break;
                    }
        ...
        public UserMessage CommandLook(Command command)
        {
            ...
            returnMessage.AddLine("I don't see");
            returnMessage.AddLine(String.Format(CultureInfo.CurrentCulture, "a {0}", command.Subject));
            returnMessage.AddLine("and I don't");
            returnMessage.AddLine("have a");
            returnMessage.AddLine(String.Format(CultureInfo.CurrentCulture, "{0}!", command.Subject));
            return returnMessage;
        }
        public UserMessage CommandTake(Command command, Point location)
        {
            ...
            {
                returnMessage.AddLine(" I don't see");
                returnMessage.AddLine(String.Format(CultureInfo.CurrentCulture, " a {0}", item.GetDescription));
                return returnMessage;
            }
            ...
            if(castleFurnature != null)
            {
                ...
                {
                    returnMessage.AddLine(String.Format(CultureInfo.CurrentCulture, "{0}", castleFurnature.GetDescription));
        ...
        public UserMessage CommandWave(Command command, Point position)
        {
            ...
            if(returnMessage == null)
            {
                ...
                if(item != null)
                {
                    ...
                    returnMessage.AddLine(String.Format(CultureInfo.CurrentCulture, "that {0}", item.GetDescription));
                }
                else
                {
                    item = FindItem(command.Subject);
        ...
        public UserMessage CommandPlay(Command command, Point position)
        {
            ...
            {
                returnMessage.AddLine(String.Format(CultureInfo.CurrentCulture, "The {0}", item.InventoryName));
